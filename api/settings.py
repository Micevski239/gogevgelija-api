...
# Security
SECRET_KEY = os.getenv("DJANGO_SECRET_KEY") or ("dev-only-do-not-use-in-prod" if os.getenv("DJANGO_DEBUG","0")=="1" else (_ for _ in ()).throw(ValueError("DJANGO_SECRET_KEY enviroment variable must be set in production")))
...
# Static / Media
# Always define STATIC_ROOT to satisfy collectstatic pre-flight checks
...
if USE_SPACES:
    ...
    STATICFILES_STORAGE = "api.settings.StaticRootS3Boto3Storage"
    DEFAULT_FILE_STORAGE = "api.settings.MediaRootS3Boto3Storage"
...
LOGGING = {
    ...
    "handlers":{
        "file":{"level":"ERROR","class":"logging.handlers.RotatingFileHandler","filename":os.path.join(BASE_DIR,"logs","django_errors.log"),
                "maxBytes":10*1024*1024,"backupCount":5,"formatter":"verbose"},
        "console":{"level":"INFO" if DEBUG else "WARNING","class":"logging.StreamHandler","formatter":"simple" if DEBUG else "verbose"},
        "security":{"level":"INFO","class":"logging.handlers.RotatingFileHandler","filename":os.path.join(BASE_DIR,"logs","security.log"),
                    "maxBytes":10*1024*1024,"backupCount":5,"formatter":"verbose"},
    },
    "loggers":{
...
CACHES = {
    "default": {
        ...
        "KEY_PREFIX": "gogevgelija","TIMEOUT": 300,
    }
}
