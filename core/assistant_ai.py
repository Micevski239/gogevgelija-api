    ...
    def generate_display_message(
        ...
        *,
        ...
        system_prompt = (
            ...
            "About Gevgelija:\n"
            "- City in southern North Macedonia, on the Greek border\n"
            "- Known for: warm climate with Mediterranean influence, hospitality, food culture\n"
            "- Nature: Kozuf Mountain (outdoor activities, fresh air, day trips), "
            "Smrdliva Voda (mineral water spring on Kozuf, peaceful setting)\n"
            ...
            "- Evening rhythm: lively cafes and restaurants, especially on weekends\n\n"
            "About the app:\n"
            ...
            "- If no results OR results are irrelevant: honestly say we don't have that in the app right now, "
...
class OpenAIAssistantAIProvider(BaseAssistantAIProvider):
    ...
    def _chat_completion(
        ...
        *,
        ...
        if not self.is_enabled():
            raise AssistantAIError("OpenAI provider is not configured")
        ...
        if response.status_code >= 400:
            ...
            raise AssistantAIError(f"OpenAI assistant call failed with status {response.status_code}")
        ...
        try:
            ...
            raise AssistantAIError("OpenAI assistant response was not valid") from exc
    ...
    def generate_greeting(
        ...
        *,
        ...
        result = self._chat_completion(messages=messages, schema_name="greeting", schema=schema)
        return result.get("greeting") or None
